...
const Header = ({
  ...
  const [isUserLoggedIn, setIsUserLoggedIn] = useState(localStorage.getItem("token") ?? false)
    ...
    >
      ...
      <div className="w-full md:w-auto flex flex-row items-center justify-between md:justify-start gap-2 min-w-[216px] max-w-full md:max-w-[300px] cursor-pointer text-13xl text-components-button-component-primarycolor font-urbanist">
        <div onClick={onHeaderClick} className="flex items-center gap-2">
          <img
            className="w-8 md:w-12 h-8 md:h-12 overflow-hidden shrink-0"
            alt=""
            src="/layer-2.svg"
          />
          <a className="text-decoration-none flex-1 tracking-[-0.02em] font-bold text-[inherit]">
            CredNation
          </a>
        </div>
        ...
        >
          {isMenuOpen ? (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="30" height="30" fill="none" stroke="currentColor" className="rounded-md">
              <g strokeWidth="2">
      ...
      {!isUserLoggedIn && <>
      {/* Menu Items */}
      <div className={`${isMenuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row w-full md:w-auto items-center justify-center gap-4 md:gap-0`}>
        <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-0">
          <div className="cursor-pointer" onClick={onButtonCompactItemClick}>
            <div className="bg-gray-200 py-2 px-6 rounded-md text-center">
              <a className="text-[inherit] leading-[24px]">Find Guards</a>
            </div>
          </div>
          <div className="cursor-pointer" onClick={onButtonCompactItemClick1}>
            <div className="bg-gray-200 py-2 px-6 rounded-md text-center">
              <a className="text-[inherit] leading-[24px]">Register as a Guard</a>
            </div>
          </div>
          <div className="cursor-pointer" onClick={onButtonCompactItemClick2}>
            <div className="bg-gray-200 py-2 px-6 rounded-md text-center">
      ...
      <div className={`${isMenuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row w-full md:w-auto items-center justify-center gap-4 md:gap-0`}>
        <div className="flex flex-col md:flex-row items-center justify-center gap-4">
          <div className="cursor-pointer w-[68px] flex items-center justify-center py-2" onClick={onButtonContainerClick}>
            <a className="text-[inherit] leading-[24px]">Login</a>
          </div>
          <div className="bg-ripe-plum-950 text-components-button-component-primarycolor rounded-lg flex items-center justify-center py-2 px-6 cursor-pointer" onClick={onButtonContainerClick1}>
      ...
      {isUserLoggedIn && <div className={`${isMenuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row w-full md:w-auto items-center justify-center gap-4 md:gap-0`}>
        <div className="flex flex-col md:flex-row items-center justify-center gap-4">
          <div className="cursor-pointer w-[68px] flex items-center justify-center py-2" style={{ border: "1px solid white", borderRadius: "6px"}} onClick={handleLogout}>
            <a className="text-[inherit] leading-[24px]">Logout</a>
          </div>
        </div>
      </div>}
    </header>
